...
export default function Create() {
  ...
  const [ createEscrowButtonText, setCreateEscrowButtonText ] = useState("Create Ecrow");
  const [formData, setFormData] = useState<{address:string, time: number, description:string, amount:string}>({
    address: "",
    time : 7,
    description : "",
    amount: ""
  ...
  const handleOnSubmit = async (e: React.SyntheticEvent) => {
    ...
    try {
      ...
      const tx = await connectedContract.createTransaction(formData.address, formData.time * 24 * 60 * 60, formData.description, options);
      await tx.wait();
      console.log(tx);
  ...
  return (
    <main className="h-screen flex flex-col gap-3 items-center justify-center">
      ...
      {
        contract && <div className="flex flex-col gap-6 text-center">
                      ...
                      <p>Connected to network: {network}</p> 
                      {
                        signer && 
                        <h1>
                          Account connected: {signer.address}
                        </h1>
                      }
                    </div>
      }
      {
        !signer &&  <button className="bg-gray-300 px-4 py-1 rounded-lg" onClick={connectWallet}>{connectButtonText}</button>
      }
      {
        signer && <form onSubmit={handleOnSubmit} className="flex flex-col gap-5">
                    <div>
                    ...
                    <button type="submit" className = "bg-customColors-secondary text-white font-bold lg:text-2xl px-6 py-3 lg:px-16 lg:py-6 rounded-lg hover:text-[#1BA9B5]">
